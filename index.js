...
Categories.forEach((Category) => {
   Fs.readdir(`./Commands/${Category}`, (error, Files) => {
      ...
      Files.forEach((File) => {
         ...
         if (!Cmd.help.name || !Cmd.help.aliases) return console.log(`${Cmd.help.name ? Cmd.help.name : "?"} ไม่สามารถโหลดได้ - ❌`);
         Client.commands.set(Cmd.help.name, Cmd);
         Cmd.help.aliases ? Cmd.help.aliases.forEach(Alias => Client.aliases.set(Alias, Cmd.help.name)) : Cmd.help.aliases = null;
         console.log(`${Cmd.help.name} โหลดแล้ว - ✅`)
      });
   });
});
...
Client.on("ready", async () => {
   console.clear();
   console.log(`บอทพร้อมเล่นเพลง - ${Client.user.username}`);
   Client.user.setActivity("Friend Music", { type: "PLAYING" });
...
Client.on("message", async (message) => {
   ...
   try {
     ...
     return message.channel.send("มีบางอย่างผิดพลาด โปรดลองอีกครั้งภายหลัง").then(() => console.log(error));
   };
});

/* Login */

Client.login(Token).catch(() => console.log(new Error("โทเค็น Bot Token ของ Discord ไม่ถูกต้อง!")));