...
async function HandleVoice(message, client) {
  ...
  await Queue.Connection.dispatcher
    ...
    .on("error", async (error) => {
      console.log(error);
      return Queue.Text.send("มีบางอย่างผิดพลาด โปรดลองอีกครั้งในภายหลัง!");
    });
};
...
async function Player(client, message, options = {}) {
  ...
  if (!options.Song) {
      ...
      .setDescription("คิวสิ้นสุด กรุณาเพิ่มเพลงเพิ่มเติม")
      .setTimestamp();
  ...
  if (options.Song.Type == "SR" || options.Song.Type == "YT") {
    ...
    if (!Data) return message.channel.send("ข้อผิดพลาด: ไม่พบเพลย์ลิสต์!");
    ...
    if (!buffer) return message.channel.send("ข้อผิดพลาด: ไม่พบเพลง!");
    Link = buffer, Type = "AR";
  };

  Steam = Type == "SR" ? await Dl.downloadFromInfo(Link, option) : Type == "AR" ? await Dl.arbitraryStream(Link, option) : undefined;
  if (!Steam) return message.channel.send("ข้อผิดพลาด: มีบางอย่างผิดพลาด โปรดลองอีกครั้งในภายหลัง!");
  ...
  if (Seek) {
    ...
    .setDescription(`กำลังเล่น - [${queue.Songs[0].Title}](${queue.Songs[0].Link})`)
